                    ...
                    <tbody>
                      @foreach ($categories as $category)
                        <tr>
                          ...
                          <td>
                              <form id="statusForm{{$category->id}}" class="d-inline-block" action="{{route('admin.faq.categories.showin')}}" method="post">
                                  @csrf
                                  <input type="hidden" name="category_id" value="{{$category->id}}">
                                  <select class=" form-control form-control-sm bg-success"
                                          name="show_in" onchange="document.getElementById('statusForm{{$category->id}}').submit();">
                                      <option value="1" {{$category->show_in == 1 ? 'selected' : ''}}>Home</option>
                                      <option value="0" {{$category->show_in == 0 ? 'selected' : ''}}>Our Story</option>
                                  </select>
                              </form>
                          </td>
                          <td>
