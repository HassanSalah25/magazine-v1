                  ...
                  <tr>
                    ...
                    <td>
                      <a class="btn btn-secondary btn-sm editbtn" href="#editModal" data-toggle="modal"
                        data-faq_id="{{$faq->id}}" data-category_id="{{ $faq->category_id }}" data-service_id="{{ $faq->service_id }}" data-question="{{$faq->question}}" data-answer="{{$faq->answer}}"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="ajaxForm" class="modal-form create" action="{{route('admin.faq.store')}}" method="POST">
          ...
          <div class="form-group">
            <label for="">Service (Optional)</label>
            <select name="service_id" id="faq_service_id" class="form-control" disabled>
              <option value="" selected>Select a service (optional)</option>
            </select>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="ajaxEditForm" class="" action="{{route('admin.faq.update')}}" method="POST">
          ...
          <div class="form-group">
            <label for="">Service (Optional)</label>
            <select name="service_id" id="inservice_id" class="form-control">
              <option value="">Select a service (optional)</option>
              @foreach ($services as $service)
                <option value="{{ $service->id }}">{{ $service->title }}</option>
              @endforeach
            </select>
            <p id="eerrservice_id" class="mb-0 text-danger em"></p>
          </div>
          <div class="form-group">
            <label for="">Question **</label>
...
<script>
  $(document).ready(function() {
    $("select[name='language_id']").on('change', function() {
      ...
      $("#faq_service_id").removeAttr('disabled');

      let langId = $(this).val();
      let categoryUrl = "{{url('/')}}/admins/faq/" + langId + "/get_categories";
      let serviceUrl = "{{url('/')}}/admins/faq/" + langId + "/get_services";
      ...
      $.get(serviceUrl, function(data) {
        let options = `<option value="" selected>Select a service (optional)</option>`;
    ...
    $('.editbtn').on('click', function() {
      ...
      $('#infaq_id').val(faqId);
      $('#incategory_id').val(categoryId);
      $('#inservice_id').val(serviceId);
      $('#inquestion').val(question);
      $('#inanswer').val(answer);
      $('#inserial_number').val(serialNumber);
    });
  });
</script>
@endsection
