      ...
      <div class="modal-body">
        ...
        <form id="ajaxForm" class="modal-form" action="{{route('admin.package.store')}}" method="POST">
          ...
          <div class="form-group {{ $categoryInfo->package_category_status == 0 ? 'd-none' : '' }}">
            ...
            <div class="form-group" id="serviceable_type_group">
                <label for="">Serviceable Type **</label>
                <select name="serviceable_type" id="serviceable_type" class="form-control">
                    <option value="">Select type</option>
                    <option value="App\Models\Service">Service</option>
                    <option value="App\Models\Scategory">Service Category</option>
                </select>
                <p id="errserviceable_type" class="mb-0 text-danger em"></p>
            </div>

            <div class="form-group" id="serviceable_id_group">
                <label for="">Serviceable Item **</label>
                <select name="serviceable_id" id="serviceable_id" class="form-control" disabled>
                    <option value="">Select item</option>
                </select>
                <p id="errserviceable_id" class="mb-0 text-danger em"></p>
            </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="selectgroup w-100">
              <label class="selectgroup-item">
                <input type="radio" name="duration" value="quarterly" class="selectgroup-input" checked>
                <span class="selectgroup-button">Quarterly</span>
              </label>
              <label class="selectgroup-item">
...
<script>
  $(document).ready(function() {
    $("select[name='language_id']").on('change', function() {
      ...
      let langId = $(this).val();
      let url = "{{url('/')}}/admins/package/" + langId + "/get_categories";
      ...
      $.get(url, function(data) {
        ...
        if (data.length == 0) {
          ...
          for (let i = 0; i < data.length; i++) {
            options +=`<option value="${data[i].id}" data-show-in-home="${data[i].show_in_home}">${data[i].name}</option>`;
          }
        }
    ...
    $("select[name='language_id']").on('change', function() {
      ...
      let url = "{{url('/')}}/admins/rtlcheck/" + $(this).val();
      // console.log(url);
      $.get(url, function(data) {
    ...
    // Function to populate serviceable_id based on serviceable_type
    function populateServiceableId() {
      var serviceableType = $('#serviceable_type').val();
      var serviceableId = $('#serviceable_id');
      var langId = $('#language').val();
      
      serviceableId.empty().append('<option value="">Select item</option>');
      
      if (serviceableType && langId) {
        serviceableId.removeAttr('disabled');
        
        if (serviceableType === 'App\\Models\\Service') {
          let url = "{{url('/')}}/admins/service/" + langId + "/get_services";
          $.get(url, function(data) {
            if (data.length == 0) {
              serviceableId.append('<option value="" disabled>No Services Exists</option>');
            } else {
              for (let i = 0; i < data.length; i++) {
                serviceableId.append('<option value="' + data[i].id + '">' + data[i].title + '</option>');
              }
            }
          });
        } else if (serviceableType === 'App\\Models\\Scategory') {
          let url = "{{url('/')}}/admins/scategory/" + langId + "/get_scategories";
          $.get(url, function(data) {
            if (data.length == 0) {
              serviceableId.append('<option value="" disabled>No Service Categories Exists</option>');
            } else {
              for (let i = 0; i < data.length; i++) {
                serviceableId.append('<option value="' + data[i].id + '">' + data[i].name + '</option>');
              }
            }
          });
        }
      } else {
        serviceableId.attr('disabled', 'disabled');
      }
    }

    // Check when serviceable_type changes
    $(document).on('change', '#serviceable_type', function() {
      populateServiceableId();
    });

    // Check when language changes
    $(document).on('change', '#language', function() {
      populateServiceableId();
    });

    // Function to toggle serviceable fields visibility based on category selection
    function toggleServiceableFields() {
      var selectedCategory = $('#package_category_id option:selected');
      var showInHome = selectedCategory.data('show-in-home');
      
      if (showInHome == 1) {
        // Hide serviceable fields for "Home Page Only" categories
        $('#serviceable_type_group').hide();
        $('#serviceable_id_group').hide();
        $('#serviceable_type').val('').trigger('change');
        $('#serviceable_id').val('').trigger('change');
      } else {
        // Show serviceable fields for other categories
        $('#serviceable_type_group').show();
        $('#serviceable_id_group').show();
      }
    }

    // Check when category changes
    $(document).on('change', '#package_category_id', function() {
      toggleServiceableFields();
    });
  });
</script>
@endsection
