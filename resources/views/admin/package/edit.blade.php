              ...
              <div class="form-group {{ $categoryInfo->package_category_status == 0 ? 'd-none' : '' }}">
                ...
                <select name="category_id" id="category_id" class="form-control">
                  ...
                  @foreach ($categories as $category)
                    <option value="{{ $category->id }}" 
                            data-show-in-home="{{ $category->show_in_home }}" 
                            {{ $category->id == $package->category_id ? 'selected' : '' }}>{{ $category->name }}</option>
                  @endforeach
                </select>
                ...
                <div class="form-group" id="serviceable_type_group">
                    ...
                    <select name="serviceable_type" id="serviceable_type" class="form-control">
                        ...
                        <option value="App\Models\Service" {{ $package->serviceable_type == 'App\Models\Service' ? 'selected' : '' }}>Service</option>
                        <option value="App\Models\Scategory" {{ $package->serviceable_type == 'App\Models\Scategory' ? 'selected' : '' }}>Service Category</option>
                    </select>
                ...
                <div class="form-group" id="serviceable_id_group">
                    ...
                    <select name="serviceable_id" id="serviceable_id" class="form-control">
              ...
              <div class="form-group">
                ...
                <div class="selectgroup w-100">
                  <label class="selectgroup-item">
                    <input type="radio" name="duration" value="quarterly" class="selectgroup-input"
                      {{$package->duration == 'quarterly' ? 'checked' : ''}}>
...
<script>
  ...
  $(document).ready(function() {
    ...
    function populateServiceableId() {
      ...
      var currentValue = '{{ $package->serviceable_id }}';
      
      serviceableId.empty().append('<option value="">Select item</option>');
      
      if (serviceableType === 'App\\Models\\Service') {
        @foreach($services as $service)
          serviceableId.append('<option value="{{ $service->id }}" {{ $package->serviceable_id == $service->id && $package->serviceable_type == "App\\Models\\Service" ? "selected" : "" }}>{{ $service->title }}</option>');
        @endforeach
      } else if (serviceableType === 'App\\Models\\Scategory') {
        @foreach($scategories as $scategory)
          serviceableId.append('<option value="{{ $scategory->id }}" {{ $package->serviceable_id == $scategory->id && $package->serviceable_type == "App\\Models\\Scategory" ? "selected" : "" }}>{{ $scategory->name }}</option>');
        @endforeach
      }
    }
    ...
    $('#serviceable_type').on('change', function() {
      populateServiceableId();
    });
