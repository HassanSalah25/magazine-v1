@foreach ($inputs as $input)
  <div class="mb-3">
    ...
    @if ($input->type == 3)
      ...
      @foreach ($input->package_input_options as $option)
        <div class="form-check form-check-inline">
          <input class="form-check-input @error($input->name) is-invalid @enderror" type="checkbox" name="{{ $input->name }}[]" id="pi{{ $option->id }}" value="{{ convertUtf8($option->name) }}" {{ in_array(convertUtf8($option->name), old($input->name, [])) ? 'checked' : '' }}>
          <label class="form-check-label" for="pi{{ $option->id }}">{{ convertUtf8($option->name) }}</label>
        </div>
      @endforeach
      @error($input->name)
        <div class="invalid-feedback d-block">{{ $message }}</div>
    ...
    @if ($input->type == 5)
      <label class="contact-label">{{ convertUtf8($input->label) }} @if ($input->required == 1)<span class="text-danger">*</span>@endif</label>
      <input type="file" class="form-control @error($input->name) is-invalid @enderror" name="{{ $input->name }}">
      <small class="text-muted">{{ __('Only zip file is allowed') }}</small>
      @error($input->name)
        <div class="invalid-feedback d-block">{{ $message }}</div>
      @enderror
    @endif
