...
if (! function_exists('replaceUrlsWithBaseUrl')) {
    function replaceUrlsWithBaseUrl($content) {
        // Replace regular asset URLs
        $content = str_replace(url('/') . '/assets/front/img/', "https://seo-wolves.com/assets/front/img/", $content);
        // Replace Summernote image URLs
        $content = str_replace(url('/') . '/assets/front/img/summernote/', "https://seo-wolves.com/assets/front/img/summernote/", $content);
        return $content;
    }
}
...
if (! function_exists('getActiveTags')) {
...
if (!function_exists('getHref') ) {
    function getHref($link) {
        ...
        } else if ($link["type"] == 'ourstory') {
            $href = route('front.ourstory');
...
if (!function_exists('getModelCategories')) {
    function getModelCategories($type) {
        return match ($type) {
            'services' => Scategory::where('status', 1)->get(),
...
if (!function_exists('getMegaMenuItems')) {
    function getMegaMenuItems($type) {
        $model = getModelName($type);
        return $model ? $model::where('feature', 1)->get() : collect();
    }
}
