...
class PackageController extends Controller
{
  ...
  public function edit(Request $request, $id)
  {
    ...
    $data['categories'] = PackageCategory::where('language_id', $lang->id)
      ...
      $data['services'] = Service::where('language_id', $lang->id)
          ->get();

      $data['scategories'] = Scategory::where('language_id', $lang->id)
          ->get();
  ...
  public function store(Request $request)
  {
    ...
    // Handle morph relation - only store if category doesn't have show_in_home = 1
    $category = PackageCategory::find($request->category_id);
    if ($category && $category->show_in_home == 1) {
        // For "Home Page Only" categories, don't store serviceable fields
        $package->serviceable_type = null;
        $package->serviceable_id = null;
    } else {
        // For other categories, store serviceable fields if provided
        if ($request->filled('serviceable_type') && $request->filled('serviceable_id')) {
            $package->serviceable_type = $request->serviceable_type;
            $package->serviceable_id = $request->serviceable_id;
        } else {
            $package->serviceable_type = null;
            $package->serviceable_id = null;
        }
  ...
  public function updateSettings(Request $request)
  {
    ...
    foreach ($bexs as $bex) {
      ...
      $bex->package_banner_image = $request->package_banner_image;
      $bex->save();
    }
