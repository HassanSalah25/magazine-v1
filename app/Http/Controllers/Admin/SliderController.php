...
class SliderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Slider::where('language_id', $lang_id);

        if ($request->has('id') && !empty($request->id)) {
            $query->where('page_type', 'landingpage' . $request->id);
            $data['landingpage_id'] = $request->id;
        } else {
            $query->where('page_type', null);
            $data['landingpage_id'] = null;
        }

        $data['sliders'] = $query->orderBy('id', 'DESC')->get();
    ...
    public function store(Request $request)
        {
            ...
            if ($request->filled('id')) {
                $be = BasicExtended::findOrFail($request->id);
                $version = $be->theme_version;
            } else {
                $be = BasicExtended::where('language_id', $request->language_id)->firstOrFail();
                $version = $be->theme_version;
            }
            ...
            if ($request->filled('id')) {
                $slider->page_type = 'landingpage' . $request->id ?? null;
            }
