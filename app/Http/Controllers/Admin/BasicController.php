...
class BasicController extends Controller
{
    ...
    public function updateseo(Request $request, $langid)
    {
        ...
        $be->our_story_meta_keywords = $request->our_story_meta_keywords;
        $be->our_story_meta_description = $request->our_story_meta_description;
        $be->tags_meta_keywords = $request->tags_meta_keywords;
        $be->tags_meta_description = $request->tags_meta_description;
        $be->save();
    ...
    public function updateheading(Request $request, $langid)
    {
        $request->validate([
            ...
            'service_shape_image' => 'nullable|max:255',
            ...
            'our_story_title' => 'nullable|max:70',
            'our_story_subtitle' => 'nullable|max:70',
            'tags_title' => 'nullable|max:70',
            'tags_subtitle' => 'nullable|max:70',
        ]);
        ...
        $bs->service_shape_image = $request->service_shape_image;
        ...
        $bex->our_story_title = $request->our_story_title;
        $bex->our_story_subtitle = $request->our_story_subtitle;
        $bex->tags_title = $request->tags_title;
        $bex->tags_subtitle = $request->tags_subtitle;
        $bex->save();
    ...
    public function updatesections(Request $request)
    {
        ...
        $bes = BasicExtended::all();
        foreach ($bes as $key => $be) {
            if (!is_null($request->pricing_section))
                $be->pricing_section = $request->pricing_section;
            if (!is_null($request->categories_section))
                $be->categories_section = $request->categories_section;
            if (!is_null($request->featured_products_section))
                $be->featured_products_section = $request->featured_products_section;
            if (!is_null($request->category_products_section))
        ...
        $bexs = BasicExtra::all();
        foreach ($bexs as $key => $bex) {
            if (!is_null($request->faq_section))
                $bex->faq_section = $request->faq_section;
            if (!is_null($request->nav_tab_section))
                $bex->nav_tab_section = $request->nav_tab_section;
            if (!is_null($request->comparison_section))
                $bex->comparison_section = $request->comparison_section;
            $bex->save();
        }
