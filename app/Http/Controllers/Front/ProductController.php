...
class ProductController extends Controller
{
    ...
    public function cart()
    {
        ...
        $tax = tax();
        $discount = coupon();
        $shipping = 0; // أو اجلب قيمة افتراضية أو من اختيار المستخدم

        $grandTotal = round(($cartTotal - $discount) + $shipping + $tax, 2);

        return view('front.product.cart', compact('cart', 'version', 'cartTotal', 'tax', 'discount', 'shipping', 'grandTotal'));
    }
    ...
    public function updatecart(Request $request)
    {
        ...
        // مثال: تحديث الكميات في السيشن أو الداتابيز حسب نظامك
        foreach ($items as $id => $qty) {
            // إذا كنت تخزن السلة في السيشن
            if (session()->has("cart.$id")) {
                session()->put("cart.$id.qty", (int)$qty);
            }
        }
    ...
    public function checkout(Request $request)
    {
        ...
        $data['stripe_key'] =  isset($stripe['key']) ? $stripe['key'] : '';
