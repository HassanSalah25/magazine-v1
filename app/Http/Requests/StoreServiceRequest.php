...
class StoreServiceRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'parent_category_id' => 'nullable|integer|exists:service_categories,id',
        ...
        foreach ($locales as $locale) {
            if ($locale === $default) {
                ...
                $rules["$locale.slug"] = ['required', 'string', 'unique:service_translations,slug', new LowercaseSlug];
            } else {
                $rules["$locale.title"] = 'nullable|string';
                $rules["$locale.slug"] = ['nullable', 'string', 'unique:service_translations,slug', new LowercaseSlug];
            }
