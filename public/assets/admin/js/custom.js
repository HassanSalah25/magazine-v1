        ...
        onImageUpload: function (files) {
          ...
          $.ajax({
            ...
            success: function (data) {
              // console.log(data);
              // Replace {base_url} placeholder with actual URL before inserting image
              var actualUrl = data.replace('{base_url}', baseurl);
              $summernote.summernote('insertImage', actualUrl);
  ...
  ******************************************************/
  ...
  window.insertImage = function(id, items) {
      items.forEach(function(item) {
          // Replace {base_url} placeholder with actual URL before inserting image
          var actualUrl = item.replace('{base_url}', baseurl);
          $("#" + id).summernote('insertImage', actualUrl);
      });
  };

  // SetUrl function for file manager integration
  window.SetUrl = function(items) {
      if (items && items.length > 0) {
          var item = items[0]; // Get the first item
          var url = item.url;
          var serial = new URLSearchParams(window.location.search).get('serial');
          ...
          if (serial) {
              ...
              $('#thumbPreview' + serial + ' img').attr('src', baseurl + '/' + url);
          }
